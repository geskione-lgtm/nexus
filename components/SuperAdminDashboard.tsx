...
const SuperAdminDashboard: React.FC<Props> = ({ activeTab }) => {
            ...
            <form onSubmit={handleSavePackage} className="grid grid-cols-1 xl:grid-cols-2 gap-12">
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <label className="text-[10px] font-medium text-text-secondary uppercase tracking-widest ml-1">Aylık Fiyat (₺)</label>
                    <input required type="number" className="w-full px-6 py-4 bg-slate-50 rounded-2xl border-none text-sm font-medium focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all" 
                      value={editingPackage.price} onChange={e => setEditingPackage({...editingPackage, price: parseInt(e.target.value)})} />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-medium text-text-secondary uppercase tracking-widest ml-1">Analiz Limiti</label>
                    <input required type="number" className="w-full px-6 py-4 bg-slate-50 rounded-2xl border-none text-sm font-medium focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all" 
    ...
    return (
      <div className="space-y-10">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
           ...
           <SoftCard className="flex flex-col">
              <p className="text-[10px] font-medium text-text-secondary uppercase tracking-[0.25em] mb-10">Lisans Dağılımı</p>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <SoftCard className="bg-indigo-600 text-white relative overflow-hidden group flex flex-col justify-between">
          ...
          <div className="relative z-10">
            <p className="text-[10px] font-medium text-white/40 uppercase tracking-widest mb-2">Sunucu Durumu</p>
            <h4 className="text-lg font-medium tracking-tight">Global Node: Aktif</h4>
            <p className="text-xs text-white/40 font-medium mt-1">Gecikme: 24ms</p>
          </div>
          <div className="relative z-10 flex items-center gap-2 mt-4">
            <div className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse" />
            <span className="text-[10px] font-medium uppercase tracking-wider text-emerald-400">Tüm Sistemler Operasyonel</span>
          </div>
        </SoftCard>
      </div>
      ...
      >
        <table className="w-full text-left">
          <thead className="text-[10px] font-medium text-text-secondary uppercase tracking-wider border-b border-border-subtle">
            <tr>
            ...
            {doctors.map(doctor => (
              <tr key={doctor.id} className="hover:bg-surface-hover transition-colors group">
                ...
                <td className="px-6 py-5 text-right">
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[9px] font-medium uppercase tracking-wider border border-emerald-100">
