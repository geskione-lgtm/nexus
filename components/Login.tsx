...
const Login: React.FC<LoginProps> = ({ onBack }) => {
  const [isLogin, setIsLogin] = useState(false); // Varsayılan Kayıt Ol
      ...
      <div className="max-w-sm w-full p-10 text-center animate-in fade-in slide-in-from-bottom-10 duration-1000">
        <div className="mb-12">
          ...
          <h2 className="text-3xl font-bold text-black tracking-tight mb-3">
            {isLogin ? 'Nexus\'a Giriş Yap' : 'Klinik Kaydı Oluştur'}
          </h2>
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          ...
          >
            {loading ? 'İşlem Yapılıyor...' : (isLogin ? 'Giriş Yap' : 'Devam Et')}
          </button>
        </form>
        ...
        <div className="mt-10">
          ...
          >
            {isLogin ? 'Yeni bir hesap oluştur' : 'Zaten bir hesabın mı var? Giriş yap'}
          </button>
        </div>
